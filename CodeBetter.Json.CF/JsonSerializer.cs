namespace CodeBetter.Json
{
    ...
    public class JsonSerializer
    {
        ...
        private void SerializeValue(object value)
        {
            ...
            if (value is int || value is long || value is short || value is float||value is decimal || value is byte || value is sbyte || value is uint || value is ulong || value is ushort || value is double)
            {
        ...
        private void SerializeObject(object @object)
        {
            ...
            List<PropertyInfo> propertys = ReflectionHelper.GetSerializablePropertys(@object.GetType());
            if (fields.Count == 0 && propertys.Count==0)
            {
                return;
            }
            ...
            {
                SerializeKeyValue(GetKeyName(propertys[0]), ReflectionHelper.GetValue(propertys[0], @object), true);
                for (int i = 1; i < propertys.Count; ++i)
                {
                    SerializeKeyValue(GetKeyName(propertys[i]), ReflectionHelper.GetValue(propertys[i], @object), false);
                }
            }
            _writer.EndObject();
