...
namespace Action.SqlServer
{
    ...
    public class SqlServerFlowAction:SqlServerBaseAction
    {
        ...
        public bool pay(Model.TradeFlow tradeFlow,Model.PayFlow payFlow)
        {
            ...
            /*
            #region 获取积分基数
            sqlStr = "select jfbase from dbo.tCzkMnyType where code=(select mclass from tczkcard where hyzh='"+card.Hyzh+"')";
            ...
            */
            ...
            sqlStr = "update tCzkCard set "
                ...
                + " xfje=xfje+(" + tradeFlow.Total.ToString("F2") + ")"
                + " where hyzh='" + payFlow.Cardno + "'";
            sqlList.Add(sqlStr);
            #endregion 

            #region 生成验证字符串
            string markstr= new CzkVerifyMark().VerifyMark(card.Hyzh,card.Total+card.Kkje-tradeFlow.Total);
            #endregion

            #region 更新验证金额
            sqlStr = "update tCzkCardMark set VerifyMark='"+markstr+"' where hykh='"+card.Hykh+"' ";
            sqlList.Add(sqlStr);
            #endregion
            ...
            if (card.JfBase > 0)
            {
                ...
                #region 更新积分
                sqlStr = "update tCzkCard set jfgrade=jfgrade+" + bcjf.ToString("F2") + " where hyzh='" + card.Hyzh + "'";
                sqlList.Add(sqlStr);
                #endregion
            ...
            */
            ...
            #region 更新储值卡进销存
            sqlStr = "exec PROC_XsCzkJxc '" + tradeFlow.Sdate + "', '" + payFlow.Cardno + "', '" + card.Hykh + "', 0, 0, "+tradeFlow.Total.ToString("F2")+", '" + tradeFlow.Operater + "', '" + tradeFlow.Operater + "', null,null";
            sqlList.Add(sqlStr);
            #endregion

            #region tradeflow
            sqlStr = "INSERT INTO pos_TradeFlow ([flow_no],[posno],[sdate],[stime],[operater],[squadno],[tradetype],[qty], [total],[zkje],[payje],[Change],[CustCode],custflag,rightno,shopno )"
            + "values("
            + "'" + tradeFlow.Flow_no+"',"
            + "'" + tradeFlow.Posno + "',"
            + "'" + tradeFlow.Sdate+"',"
            ...
            #region payflow
            sqlStr = "insert into pos_PayFlow ([flow_no],[flow_id],[posno],[sdate],[stime],[operater],[squadno],[tradetype],[paypmt],[PayAmount],[total] ,[cardno],[scye],[ckic],[shopno])"
            + " values( "
            + "'" + payFlow.Flow_no + "',"
        ...
        public bool saveFlow(List<Model.TradeFlow> tradeFlowList, List<Model.PayFlow> payFlowList)
        {
            ...
            {
                sqlStr = "INSERT INTO pos_TradeFlow ([flow_no],[djlx],[posno],[sdate],[operater],[squadno],[tradetype],"
            + " [total],[zkje],[payje]"
            ...
            {
                sqlStr = "insert into pos_PayFlow ([flow_no],[flow_id],[posno],[sdate],[operater],[squadno],"
