...
namespace Action.Sqlite
{
    ...
    public class SqliteUserAction:SqliteBaseAction
    {
        ...
        public Model.User userLogon(string userCode, string pwd)
        {
            ...
            #region 组装sql语句
            string sqlStr = "select * from xtmuser where userCode='" + userCode + "' and mpassword='" + pwd + "'";
            #endregion
            try
            {
        ...
        public bool saveUser(List<Model.User> userList)
        {
            ...
            foreach (Model.User user in userList)
            {
                string sqlStr = "insert into xtmuser (userCode,userName,mpassword,mclass,mdept,rightCode) "+
                    "values('" + user.UserCode + "','" + user.UserName + "','" + user.Mpassword + "','" + user.Mclass.ToString() + "','" + user.Mdept + "','" + user.RightCode + "')";
