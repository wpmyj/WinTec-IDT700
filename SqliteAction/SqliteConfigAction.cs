    ...
    public class SqliteConfigAction:SqliteBaseAction
    {
        ...
        public Model.Config getConfig()
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    config.DeptName = reader["DeptName"].ToString();
                    config.printBill = reader["printBill"].ToString();
                    config.readCardMode = reader["ReadCardMode"].ToString();
                    config.offLineSale = reader["offLineSale"].ToString();
                    config.DataSaveDays = int.Parse(reader["dataSaveDays"].ToString());
        ...
        public bool saveConfig(Model.Config config)
        {
            ...
            string sqlStr = "update Config set "
                ...
                + "offLineSale='"+config.offLineSale+"',"
                +"dataSaveDays="+config.DataSaveDays+","
                +"str1='"+config.Str1+"',"
                +"str2='"+config.Str2+"',"
                +"str3='"+config.Str3+"'";
